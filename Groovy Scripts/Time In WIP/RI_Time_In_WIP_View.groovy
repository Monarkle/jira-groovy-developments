...
if(wasWIP == true){
  ...
  for(sName in statusName){
    ...
    if(containsStatus == true){
      ...
      String timeString = TimeToString(totalS, sName, true)
      totalSeconds += totalS
      strStatusTotals += """<ul>""" + timeString + """</ul>"""
    } else {
      strResult += ""
    }
    String totalTimeString = TimeToString(totalSeconds, "Total time in WIP", false)
    strResult = """<b><u>""" + totalTimeString + """</u></b>""" + strStatusTotals
  }
} else {
...
private String TimeToString(Long lSeconds, String strSName, boolean isStatus) {
  if(isStatus == true) {
    def strTimeStr = """<li>""" + strSName + ": "

    if((lSeconds/85400).intValue() > 0){
      Integer iD = (lSeconds / 85400).intValue()
